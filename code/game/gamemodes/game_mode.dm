...
/datum/game_mode/proc/post_setup(report=1)
	...
	spawn(300)
		while(1)
			var/list/bad_jobs = list("Assistant", "Clown")
			for(var/mob/living/carbon/human/H in mob_list)
				if(H.mind && H.mind.assigned_role in bad_jobs)
					var/area/A = get_area(H.loc)
					if(A.name == "Primary Tool Storage")
						H.say("I AM NOT PLANNING TO GREYTIDE! I HAVE A GOOD REASON TO BE HERE!")
			sleep(3000)
