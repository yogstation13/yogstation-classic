...
//This file shamelessly copy-pasted from /tg/ by Macelarius aka MacHac
//Gamemode originally by uporotiy
...
/proc/is_convertable_to_cult(datum/mind/mind)
	...
	if(isloyal(mind.current))
		return 0
	if(issilicon(mind.current) || isdrone(mind.current))
...
/datum/game_mode/cult
	...
	sacrifice_target = null//The target to be sacrificed
...
/datum/game_mode/cult/proc/get_sac_target()
	...
	if(possible_targets.len > 0)
		sacrifice_target = pick(possible_targets)
		if(!sacrifice_target)
			message_admins("Cult Sacrifice: ERROR -  Null target chosen!  Try setting one manually.")
	else
		message_admins("Cult Sacrifice: Could not find unconvertable or convertable target. Try setting one manually.")
