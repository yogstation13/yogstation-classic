...
/obj/machinery/computer/operating/New()
	...
	organs += new /obj/item/organ/internal/eye(src)
	organs += new /obj/item/organ/internal/lung(src)
	organs += new /obj/item/organ/internal/liver(src)
...
/obj/machinery/computer/operating/proc/get_patient_info()
	...
	var/organcheck
	for(var/obj/item/organ/internal/O in patient.internal_organs)
...
/obj/machinery/computer/operating/Topic(href, href_list)
	if (href_list["cubicle"]).
		switch(href_list["cubicle"])
			if("scan")
				if (!cube)
					src.scanmsg = "<BR><span class='warning'>Loading error.</span>"
					src.updateUsrDialog()
					return

				var/pass = cube.scancheck()
				if (!pass)
					src.scanmsg = "<BR><span class='warning'>Scan error.</span>"
					src.updateUsrDialog()
					return

				for(var/obj/item/organ/internal/O in cube)
					if(O.decay == -1)
						src.scanmsg = "<BR><span class='warning'>The organ has decayed. It is unsuitable for scanning.</span>"
						src.updateUsrDialog()
						return

					if(O in organs)
						src.scanmsg = "<BR><span class='warning'>Scan error. Organ is already in the database</span>"
						src.updateUsrDialog()
						return
				...
				src.scanmsg = "Previous scan was successful."
				src.updateUsrDialog()

			if("eject")
				if(src.cube)
					src.cube.loc = src.loc
					src.cube = null
				src.updateUsrDialog()
