#define FLOODLAMP_POWER_USAGE 10 // 10 is good for per lamp for now, hopefully..
...
obj/machinery/flood_lamp/process()
	if(istype(cell) && on)
		if(!cell.use(flashlights.len * FLOODLAMP_POWER_USAGE)) // If we run out of power, turn off. No Freebies bitches.
			on = 0
			update_all()
...
obj/machinery/flood_lamp/proc/toggle_light()
	if(on)
		on = 0
	else if (istype(cell) && cell.charge > flashlights.len * FLOODLAMP_POWER_USAGE) // We can't turn on if we don't have enough power.
		on = 1
	update_all()
