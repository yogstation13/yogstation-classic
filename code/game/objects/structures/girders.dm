	...
	if(istype(W, /obj/item/weapon/tool/screwdriver))
				...
				if(state == GIRDER_DISPLACED)
					...
					if(do_after(user, 20, target = src))
						if(!src.loc || !S || S.get_amount() < 2)
							return
						S.use(2)
						adm_action_log.enqueue("[gameTimestamp()] ([user] - [W] - [src]): created")
					...
					if (do_after(user, 40, target = src))
						if(loc == null || S.get_amount() < 2)
							return
						S.use(2)
						adm_action_log.enqueue("[gameTimestamp()] ([user] - [W] - [src]): plating added")
				...
				if(state == GIRDER_DISPLACED)
					...
					if(do_after(user, 20, target = src))
						if(!src.loc || !S || S.amount < 2)
							return
						S.use(2)
						adm_action_log.enqueue("[gameTimestamp()] ([user] - [W] - [src]): reinforced wall created")
					...
					if(state == GIRDER_REINF)
						...
						if(do_after(user, 50, target = src))
							if(!src.loc || !S || S.amount < 1)
								return
							S.use(1)
							adm_action_log.enqueue("[gameTimestamp()] ([user] - [W] - [src]): fully reinforced")
		...
		if(S.sheettype)
			...
			if(state == GIRDER_DISPLACED)
				if(S.amount < 2)
					...
					return
				S.use(2)
				adm_action_log.enqueue("[gameTimestamp()] ([user] - [W] - [src]): wall created")
				user << "<span class='notice'>You create a false wall. Push on it to open or close the passage.</span>"
		...
		if (P.pipe_type in list(0, 1, 5))	//simple pipes, simple bends, and simple manifolds.
			if(!user.drop_item())
				return
			P.loc = src.loc
			adm_action_log.enqueue("[gameTimestamp()] ([user] - [W] - [src]): pipe fitted")
...
/obj/structure/cultgirder/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/weapon/tool/weldingtool))
		...
		var/obj/item/weapon/pickaxe/drill/jackhammer/D = W
		adm_action_log.enqueue("[gameTimestamp()] ([user] - [W] - [src]): smashed through")
