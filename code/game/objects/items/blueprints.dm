...
/obj/item/areaeditor/proc/create_area()
	...
	message_admins("[key_name_admin(usr)]<A HREF='?_src_=holder;adminmoreinfo=\ref[usr]'>?</A> (<A HREF='?_src_=holder;adminplayerobservefollow=\ref[usr]'>FLW</A>) created new area '[A.name]'. ([loc.x],[loc.y],[loc.z])")
