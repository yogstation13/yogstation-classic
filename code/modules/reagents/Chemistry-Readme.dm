...
About the Holder:
	...
	currently in the object.It also has all the procs needed to manipulate reagents
		...
		reaction(var/atom/A, var/method=TOUCH, var/volume_modifier=0)
			...
			us if the mob TOUCHed the reagent, if it INGESTed the reagent, if the
			reagent was VAPORIZEd on them, if the reagent was INJECTed or transfered via a PATCH to them.
