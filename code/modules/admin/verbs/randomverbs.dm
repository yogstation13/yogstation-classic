...
/proc/create_xeno(ckey)
	if(!ckey)
		...
		for(var/mob/M in player_list)
			...
			if(!M.client.prefs.hasSpecialRole(BE_XENOMORPH))	continue	//we don't want to be an alium
	...
	if(G_found.mind && !G_found.mind.active)	//mind isn't currently in use by someone/something
		...
		//check if they were a monkey
			...
			if(alert("This character appears to have been a monkey. Would you like to respawn them as such?",,"Yes","No")=="Yes")
				...
				message_admins("<span class='adminnotice'>[key_name_admin(usr)] has respawned [new_monkey.key] as a filthy monkey.</span>")
				return	//all done. The ghost is auto-deleted
...
/client/proc/cmd_admin_create_faction_announcements()
	...
	if(!holder)
		src << "Only administrators may use this command."
		return
	var/faction = alert(src, "Who's there?", "Knock knock", "Centcom", "Other", "Custom")
	if(faction == "Centcom")
