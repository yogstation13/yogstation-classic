...
//ADMINVERBS
/client/proc/investigate_show( subject in list("hrefs","notes","ntsl","singulo", "chemistry", "multikey", "supermatter", "wires","telesci", "gravity", "atmos", "records", "cargo", "viro", "pda", "experimentor", "kudzu") )
	set name = "Investigate"
	set category = "Admin"
	if(!holder)	return
	switch(subject)
		if("singulo", "chemistry", "supermatter", "ntsl", "wires", "telesci", "gravity", "atmos", "records", "cargo", "viro", "pda", "experimentor", "kudzu")			//general one-round-only stuff
			var/F = investigate_subject2file(subject)
			if(!F)
