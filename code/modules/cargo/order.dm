...
/datum/supply_order/New(datum/supply_packs/pack, orderer, orderer_rank, orderer_ckey, reason)
...
/datum/supply_order/proc/generate(turf/T)
	...
	if(M.errors & MANIFEST_ERROR_ITEM)
		if(istype(C, /obj/structure/closet/crate/secure) || istype(C, /obj/structure/largecrate))
