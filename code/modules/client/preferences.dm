...
//some autodetection here.
	...
	"revenant",										//15
	"zombie",										//16
	"cyberman" = /datum/game_mode/cybermen			//17
)
...
/datum/preferences
	...
	var/ooccolor = null
	var/list/antag_flags = list()			//Special role selection
...
/datum/preferences/proc/has_antag_pref(pref)
	return (antag_flags[get_proper_antag_flag_index(pref)] & (2**(pref%16)))

/datum/preferences/proc/toggle_antag_pref(pref)
	antag_flags[get_proper_antag_flag_index(pref)] ^= 2**(pref%16)

/datum/preferences/proc/get_proper_antag_flag_index(pref)
	var/index = round(pref/16)+1
	while(antag_flags.len < index)
		antag_flags += 0
	return index
				...
				for (var/i in special_roles)
					if(jobban_job_in_list(bans, i))
						...
						if(days_remaining)
							...
							dat += "<b>Be [i]:</b> <a href='?_src_=prefs;preference=be_special;num=[n]'>[has_antag_pref(n) ? "Yes" : "No"]</a><br>"
					n++
			if(is_donator(user.client))
...
/datum/preferences/proc/SetChoices(mob/user, limit = 20, list/splitJobs = list("Atmospheric Technician"), widthPerColumn = 295, height = 620)
	if(!SSjob)	return
