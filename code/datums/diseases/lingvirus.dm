...
/datum/disease/lingvirus/stage_act()
	...
	switch (stage)
		if (1)
			if (prob(2))
				affected_mob.hallucination = min(15, affected_mob.hallucination+2)
			if (prob(3))
		...
		if (3)
			if (prob(7))
				affected_mob.hallucination = min(15, affected_mob.hallucination+7)
		...
		if (4)
			if (prob(1))
				affected_mob.hallucination = min(9, affected_mob.hallucination+1)
			if (prob(3) && affected_mob.hallucination <= 8)
