 ...
 * Command line-related refactoring by george99g
 *
...
bool writeToFile(const std::string& outputFilePath)
{
    ...
    if(outputFile.is_open())
    {
        ...
        if(totalInfiniteLoops > 0)
        {
            ...
            for (int i=0; i < (int)infiniteLoops.size(); i++)
            {
        ...
        if(totalHardDels > 0)
        {
            ...
            for(int i=0; i < (int)hardDels.size(); i++)
            {
...
int main(int argc, char** argv)
{
    ...
    {
        if(std::string(argv[i]) == "-h" || std::string(argv[i]) == "--help" || argc < 2)
        {
            std::cout<<"Usage: RuntimeCondenser.exe [OPTIONS] [INPUT FILE]\n"
                       "  -h, --help     display this help text and exit\n"
                       "  -o, --output   specify an output file\n"
                       "Example: RuntimeCondenser.exe --output output.txt input.txt\n";
            return 0;
        }
        else if(std::string(argv[i]) == "-o" || std::string(argv[i]) == "--output")
        {
            if(i >= argc - 1)
            {
                std::cerr<<"--output requires one argument.\n";
                return 1;
            }
    ...
    if(writeToFile(output))
        std::cout<<"Successfully wrote to output file.\n";
    else
    {
